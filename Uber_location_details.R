...
library(dplyr)
library(tidyr) # For Filtering dataset
...
# rbind (row-bind) is used to combine different dataframes or lists...
cmb_data <- rbind(apr_data, may_data, jun_data, jul_data, aug_data, sep_data)

# Clearing unused variables or deleting objects... And freeing up unsed Memory...
rm(apr_data)
rm(may_data)
rm(jun_data)
rm(jul_data)
...
# factor It stores the data as a vector of integer values
# label: TRUE will display the month as a character string such as "January."
...
#  reduces a data frame to a summary of just one vector or value
hour_data <- cmb_data %>% group_by(hour) %>% dplyr::summarize(Total = n())

# Bar Graph of hour_data... 
...
ggplot(month_group, aes(month, Total)) + 
  geom_bar(stat = "identity", fill = "orange") + # It leaves the data as is
